  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (ext === '.geojson' || ext === '.json') {
        ...
        // ✅ اطمینان از فرمت صحیح FeatureCollection
        ...
        console.log('✅ فایل GeoJSON پردازش شد. تعداد ویژگی‌ها:', featureCollection.features.length);
        console.log('📊 داده ارسالی به onUpload:', featureCollection);
        ...
        features: [
          {
            ...
            properties: {
              name: 'محدوده معدن',
              type: 'boundary',
              description: 'این یک محدوده نمونه است'
            }
          }
        ]
      };
      console.log('⚠️ فایل غیر GeoJSON با داده نمونه پردازش شد');
            ...
            <div className={`px-6 py-4 border-b flex items-center justify-between flex-shrink-0 ${
              ...
              <div className="flex items-center gap-3">
                <div className={`w-8 h-8 rounded-xl flex items-center justify-center ${
                  ...
                  <CloudArrowUpIcon className={`w-4 h-4 ${isDark ? 'text-[#C9A227]' : 'text-[#C9A227]'}`} />
                </div>
                <div>
                  <h3 className={`text-sm font-bold ${isDark ? 'text-white' : 'text-gray-800'}`}>📤 بارگذاری نقشه معدن</h3>
                  <p className={`text-[10px] ${isDark ? 'text-[#4A6A8A]' : 'text-gray-400'}`}>آپلود فایل‌های GeoJSON، DXF یا Shapefile</p>
                </div>
              </div>
