  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (isEditing && editingBlock) {
        ...
        ActivityLogger.logBlockUpdate(
          updatedBlock, 
          undefined, 
          ['code', 'targetLevel', 'blockNumber', 'totalHoles'], 
          editingBlock, 
          updatedBlock
        );
        result = updatedBlock;
        ...
        if (result) {
          ActivityLogger.logBlockCreate(result);
        }
      }
    ...
    },
    {
      ...
      render: (item: FullBlock) => (
        <div className="flex items-center gap-1.5">
          ...
          {/* ✅ دکمه چرخه ساب‌بلوک‌ها و خطوط خردایش */}
          <button
            onClick={() => {
              navigate(`/mining-lifecycle?blockId=${item.id}`);
            }}
            className={`p-1.5 rounded-lg transition-colors ${
              isDark ? 'hover:bg-[#00D4FF]/20 text-[#00D4FF]' : 'hover:bg-cyan-100 text-cyan-700'
            }`}
            title="چرخه ساب‌بلوک‌ها و خطوط خردایش"
          >
            ...
            <button
              onClick={() => {
                if (window.confirm(`آیا از حذف بلوک ${item.code} اطمینان دارید؟`)) {
                  ActivityLogger.logBlockDelete(item.code, undefined, item);
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className="px-4 py-2 bg-gradient-to-r from-[#00D4FF] to-[#0099CC] text-[#0A1628] font-bold rounded-xl hover:shadow-lg hover:shadow-[#00D4FF]/25 transition-all flex items-center gap-2 cursor-pointer text-sm"
            >
              <span>🔬 چرخه ساب‌بلوک‌ها و خطوط خردایش</span>
            </button>
            <button
              onClick={() => {
              ...
              className="px-4 py-2 bg-[#C9A227] text-[#1A2A3A] font-bold rounded-xl hover:bg-[#D4AF37] transition-colors flex items-center gap-2 cursor-pointer text-sm"
            >
