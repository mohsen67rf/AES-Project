...
export function initializeRepositories(): void {
  ...
  // تضمین وجود بلوک شاخص 1040 B 33 به همراه مشخصات کامل مهندسی و ساب‌بلوک‌ها
  const hasBlock1040B33 = BlockRepository.getAll().some(b => b.code.replace(/\s+/g, '').toUpperCase() === '1040B33');
  if (!hasBlock1040B33) {
    const blockB33: Block = {
      id: 'block-1040-b33',
      code: '1040 B 33',
      name: 'بلوک استخراجی ۱۰۴۰-۳۳ (پله ۱۰۴۰ - زون شرقی)',
      targetLevel: 1040,
      blockNumber: 33,
      drillingParams: {
        totalHoles: 52,
        holeDiameter: 76,
        avgDesignDepth: 12.5,
        pattern: 'شبکه مثلثی ۳×۳.۵ متر',
      },
      geometry: {
        type: 'Polygon',
        coordinates: [[[54.37, 31.52], [54.42, 31.52], [54.42, 31.58], [54.37, 31.58], [54.37, 31.52]]],
        drillingPoints: [],
      },
      status: 'APPROVED',
      statusHistory: [
        { status: 'DEFINED', changedBy: 'مهندس رضایی (پیمانکار)', changedAt: new Date(Date.now() - 86400000 * 5).toISOString() },
        { status: 'APPROVED', changedBy: 'دکتر علوی (نظارت)', changedAt: new Date(Date.now() - 86400000 * 4).toISOString(), reason: 'تأیید شبکه چال‌زنی و صدور پرمیت' },
        { status: 'DRILLED', changedBy: 'واحد حفاری', changedAt: new Date(Date.now() - 86400000 * 3).toISOString() },
        { status: 'SAMPLING_COMPLETED', changedBy: 'مهندس حسینی', changedAt: new Date(Date.now() - 86400000 * 2).toISOString() },
      ],
      createdBy: 'user-mining-1',
      createdAt: new Date(Date.now() - 86400000 * 5).toISOString(),
      updatedAt: new Date().toISOString(),
    };
    BlockRepository.save(blockB33);

    const drillPatternB33: DrillPatternDesign = {
      id: 'dp-1040-33',
      blockId: blockB33.id,
      bandId: 'band-1405-03-01',
      code: 'DP-1040-B33-V1',
      blockCode: '1040 B 33',
      designerContractor: 'دفتر فنی پیمانکار استخراج',
      holeDiameterMm: 76,
      burdenMeters: 3.0,
      spacingMeters: 3.5,
      subDrillingMeters: 1.0,
      holeCount: 52,
      avgDepthMeters: 12.5,
      totalMetersDesign: 650,
      explosiveType: 'آنفو (ANFO) + بوستر ۵۰۰ گرمی Emulite',
      powderFactorKgPerM3: 0.68,
      status: 'PERMIT_ISSUED',
      supervisionReviewer: 'دکتر علوی (دفتر فنی نظارت)',
      supervisionNotes: 'طرح حفاری و انطباق با مرز سنگ‌آهن تأیید شد.',
      permitNumber: 'PERMIT-DRL-1405/03/33',
      permitIssuedAt: new Date(Date.now() - 86400000 * 4).toISOString(),
      createdAt: new Date(Date.now() - 86400000 * 5).toISOString(),
      updatedAt: new Date().toISOString(),
    };
    DrillPatternDesignRepository.save(drillPatternB33);

    const nowTime = new Date();
    const subBlocksB33: SubBlock[] = [
      {
        id: 'sb-1040-33-sa',
        blockId: blockB33.id,
        code: '1040 B 33 – SA',
        ...
        tonnage: 5200,
        estimatedTonnage: 5000,
        sampleId: 'SMP-1040-33-SA',
        sampleNumber: 'SMP-1040-33-SA',
        sampleType: 'POWDER_BLASTHOLE',
        sampler: 'مهندس حسینی (نظارت)',
        sampleDate: new Date(nowTime.getTime() - 86400000 * 3).toISOString(),
        labResults: {
          fe: 61.8,
          feo: 21.4,
          sio2: 4.1,
          ...
          assay: 61.8,
          labName: 'آزمایشگاه مرکزی مجتمع (XRF)',
          batchNumber: 'XRF-1405-33-A',
          labTechnician: 'دکتر علوی',
          analyzedAt: new Date(nowTime.getTime() - 86400000 * 2).toISOString(),
          isVerified: true,
        },
        materialClass: 'سنگ‌آهن مگنتیت پرعیار ممتاز (High-Grade DSO)',
        rockType: 'مگنتیت متراکم',
        oreType: 'کانسنگ مگنتیتی',
        gradeCategory: 'HIGH',
        economicClass: 'DSO مستقیم',
        destination: 'CRUSHER_LINE_1',
        destinationReason: 'عیار ۶۱.۸٪ با عیار گوگرد بسیار پایین - تخصیص به خط ۱ سنگ‌شکن فکی',
        statusHistory: [
          { status: 'DEFINED', changedAt: new Date(nowTime.getTime() - 86400000 * 5).toISOString(), changedBy: 'پیمانکار' },
          { status: 'SAMPLING_COMPLETED', changedAt: new Date(nowTime.getTime() - 86400000 * 3).toISOString(), changedBy: 'مهندس حسینی' },
          { status: 'LAB_VERIFIED', changedAt: new Date(nowTime.getTime() - 86400000 * 2).toISOString(), changedBy: 'آزمایشگاه مرکزی' },
          { status: 'GRADE_CLASSIFIED', changedAt: new Date(nowTime.getTime() - 86400000 * 1.5).toISOString(), changedBy: 'مهندس کریمی' },
          { status: 'FINAL_PRODUCT', changedAt: new Date(nowTime.getTime() - 86400000 * 1).toISOString(), changedBy: 'دیسپاچینگ' },
        ],
        createdBy: 'user-mining-1',
        createdAt: new Date(nowTime.getTime() - 86400000 * 5).toISOString(),
        updatedAt: new Date().toISOString(),
      },
      {
        id: 'sb-1040-33-sb',
        ...
        benchLevel: 1040,
        tonnage: 4600,
        estimatedTonnage: 4400,
        sampleId: 'SMP-1040-33-SB',
        sampleNumber: 'SMP-1040-33-SB',
        sampleType: 'POWDER_BLASTHOLE',
        sampler: 'مهندس حسینی',
        sampleDate: new Date(nowTime.getTime() - 86400000 * 3).toISOString(),
        labResults: {
          ...
          assay: 57.2,
          labName: 'آزمایشگاه مرکزی مجتمع (XRF)',
          batchNumber: 'XRF-1405-33-B',
          labTechnician: 'دکتر علوی',
          analyzedAt: new Date(nowTime.getTime() - 86400000 * 2).toISOString(),
          isVerified: true,
        },
        materialClass: 'سنگ‌آهن مگنتیت متوسط‌عیار (Medium-Grade)',
        rockType: 'مگنتیت بین‌لایه',
        oreType: 'کانسنگ مگنتیتی',
        gradeCategory: 'MEDIUM',
        economicClass: 'خوراک دپوی پرعیار و پرعیارسازی',
        destination: 'HIGH_GRADE_STOCKPILE',
        destinationReason: 'عیار ۵۷.۲٪ با گوگرد کنترل‌شده - ارسال به دپوی پرعیار شماره ۱',
        statusHistory: [
          { status: 'DEFINED', changedAt: new Date(nowTime.getTime() - 86400000 * 5).toISOString(), changedBy: 'پیمانکار' },
          { status: 'SAMPLING_COMPLETED', changedAt: new Date(nowTime.getTime() - 86400000 * 3).toISOString(), changedBy: 'مهندس حسینی' },
          { status: 'LAB_VERIFIED', changedAt: new Date(nowTime.getTime() - 86400000 * 2).toISOString(), changedBy: 'آزمایشگاه مرکزی' },
      ...
      },
      {
        ...
        code: '1040 B 33 – SC',
        sequence: 3,
        status: 'GRADE_CLASSIFIED',
        benchLevel: 1040,
        tonnage: 3800,
        estimatedTonnage: 3600,
        sampleId: 'SMP-1040-33-SC',
        sampleNumber: 'SMP-1040-33-SC',
        sampleType: 'POWDER_BLASTHOLE',
        sampler: 'مهندس اکبری',
        sampleDate: new Date(nowTime.getTime() - 86400000 * 3).toISOString(),
        labResults: {
          ...
          assay: 46.5,
          labName: 'آزمایشگاه مرکزی مجتمع (XRF)',
          batchNumber: 'XRF-1405-33-C',
          labTechnician: 'مهندس نوری',
          analyzedAt: new Date(nowTime.getTime() - 86400000 * 2).toISOString(),
          isVerified: true,
        },
        materialClass: 'سنگ‌آهن کم‌عیار هماتیتی-مگنتیتی (Low-Grade)',
        rockType: 'اسکارن کانی‌سازی شده',
        oreType: 'کانسنگ مگنتیت-هماتیت',
        gradeCategory: 'LOW',
        economicClass: 'دپوی کم‌عیار کارخانه فرآوری',
        destination: 'LOW_GRADE_STOCKPILE',
        destinationReason: 'عیار ۴۶.۵٪ - نیازمند عیارافزایی در مدار فلوتاسیون و جدایش مغناطیسی',
        statusHistory: [
          { status: 'DEFINED', changedAt: new Date(nowTime.getTime() - 86400000 * 5).toISOString(), changedBy: 'پیمانکار' },
          { status: 'SAMPLING_COMPLETED', changedAt: new Date(nowTime.getTime() - 86400000 * 3).toISOString(), changedBy: 'مهندس اکبری' },
          { status: 'LAB_VERIFIED', changedAt: new Date(nowTime.getTime() - 86400000 * 2).toISOString(), changedBy: 'آزمایشگاه' },
          { status: 'GRADE_CLASSIFIED', changedAt: new Date(nowTime.getTime() - 86400000 * 1).toISOString(), changedBy: 'زمین‌شناسی' },
        ],
        createdBy: 'user-mining-1',
        createdAt: new Date(nowTime.getTime() - 86400000 * 5).toISOString(),
        updatedAt: new Date().toISOString(),
      },
      {
        ...
        benchLevel: 1040,
        tonnage: 3100,
        estimatedTonnage: 3000,
        sampleId: 'SMP-1040-33-SD',
        sampleNumber: 'SMP-1040-33-SD',
        sampleType: 'POWDER_BLASTHOLE',
        sampler: 'مهندس اکبری',
        sampleDate: new Date(nowTime.getTime() - 86400000 * 2).toISOString(),
        materialClass: 'باطله برونزون پیت ۱۰۴۰ (Waste Rock)',
        rockType: 'هورنفلس و آهک دگرگون',
        oreType: 'باطله استخراجی',
        gradeCategory: 'WASTE',
        economicClass: 'باطله باربری به دپوی باطله',
        destination: 'WASTE_DUMP',
        destinationReason: 'زون عقبی پله ۱۰۴۰ - انتقال مستقیم به دپوی باطله غربی',
        statusHistory: [
          { status: 'DEFINED', changedAt: new Date(nowTime.getTime() - 86400000 * 5).toISOString(), changedBy: 'پیمانکار' },
          { status: 'SAMPLING_COMPLETED', changedAt: new Date(nowTime.getTime() - 86400000 * 2).toISOString(), changedBy: 'مهندس اکبری' },
        ],
        createdBy: 'user-mining-1',
        createdAt: new Date(nowTime.getTime() - 86400000 * 5).toISOString(),
        updatedAt: new Date().toISOString(),
    ...
    SubBlockRepository.saveBatch(subBlocksB33);
    console.log('✅ بلوک و ساب‌بلوک‌های استاندارد 1040 B 33 ثبت شدند');
  }
}
